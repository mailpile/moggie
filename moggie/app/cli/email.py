...
#
# TODO:
#   - moggie send -- use this within moggie email
#   - moggie annotate -- use within moggie send and moggie email, to track sending
#                        progress and delayed-sending preferences.
#
...
from ...util.mailpile import b64c, sha1b64
...
class CommandEmail(CLICommand):
    ...
    ## Message directories / drafts

    Messages can be assembled based on the contents of an existing e-mail,
    or a collection of files found in a local directory (using the same
    structure as generated by `moggie search --format=msgdirs`).

    ### Examples:

        # 1. Use an existing e-mail as a draft, but change the from address
        moggie email --draft=id:234 --from=user@example.org

        # 2. The same thing, but creating a message-directory as an
        #    intermediate step, so attachments can be added or content edited
        #    interactively:
        moggie search id:234 --format=msgdirs --indent='' \
            |tar xfz - --strip-components=3

        # ... make changes ...

        moggie email --draft=. --from=user@example.org

    In all cases, explicit command-line arguments will override whatever is
    found in the draft messages. Also note that the structure of the draft
    message is largely ignored; text and HTML parts are detected and used,
    as are attachments, but the order or MIME encoding details.

    The contents of a directory may become internally inconsistent, if the
    user adds/removes files or edits the `message.txt` by hand. These are
    resolved using the following rules:

       1. If present the `structure.json` defines initial headers and MIME
          structure, otherwise the message starts off empty.
       2. If attachments are deleted from the directory, they are removed
          from the message (even if mentioned in `structure.json`).
       3. Files not already mentioned are added as attachments.
       4. If the file `message.txt` exists and is newer than `structure.json`,
          then it is intrpreted as a human-readable approximation of a classic
          RFC822 e-mail using utf-8 encoding. That is, first a set of headers,
          followed by a blank line and finally a text message. The headers
          present in `message.txt` will be used instead of those found in
          `structure.json`, and the message text will be used to replace the
          plain-text message body interpreted as markdown to synthesize an
          HTML part as well.

    *TODO:* Decide and document how drafts specify encryption features
    and how we generate HTML mail with inline images. Decide and document how
    the user can control/disable HTML generation.
...
#FIXME: ('--template=',       [], 'Search terms, filesystem path or - for stdin'),
    ...
    def _make_boundary(self):
        # We derive boundaries from the Message-Id in a stable way, to reduce
        # variability in our output - in particular when editing drafts.
        if 'message-id' in self.headers:
            self.boundary_count += 1
            return b64c(sha1b64('%s-%d' % (
                self.headers['message-id'],
                self.boundary_count)))
        return b64c(sha1b64(os.urandom(32)))

    def _load_msgdir(self, dirname):
        from moggie.storage.exporters.msgdirs import ImportDirectory
        return ImportDirectory(*os.path.split(os.path.abspath(dirname)))
    ...
    def configure(self, args):
        ...
        def as_file(key, i, t, target, file_reader, dir_reader=None):
            if t[:1] == '-':
                ...
                target.append(file_reader(sys.stdin.buffer))
                self.options[key][i] = None
            elif os.path.exists(t):
                if os.path.isdir(t) and dir_reader is not None:
                    target.append(dir_reader(t))
                else:
                    with open(t, 'rb') as fd:
                        target.append(file_reader(fd))
                self.options[key][i] = None
...
<p class="fwdMetainfo">-------- Original Message --------<br>\n%s</p>"""
    ...
    async def update_email_tree(self):
        return self.email_tree

    async def render_email(self, as_tree):
        ...
        if 'date' not in self.headers:
            ...
            now_ts = (time.time() // 60) * 60  # Reduce resolution to minutes
            self.headers['date'] = [datetime.datetime.fromtimestamp(now_ts)]
    ...
    def gather_draft_data(self):
        ...
        def _ai(ai):
            if isinstance(ai, AddressInfo):
                return ai
            return AddressInfo(ai['address'], ai['fn'])

        for draft in self.drafts:
            if (not self.options['--subject=']) and draft.get('subject'):
                self.options['--subject='].append(draft['subject'])

            if (not self.options['--from=']) and draft.get('from'):
                self.options['--from='].append(_ai(draft['from']))

            for opt, hdr in (
                    ('--to=', 'to'),
                    ('--cc=', 'cc'),
                    ('--bcc=', 'bcc')):
                if (not self.options[opt]) and draft.get(hdr):
                    for e in draft[hdr]:
                        self.options[opt].append(_ai(e))

            misc_headers = []
            for hdr in draft:
                if (hdr[:1] != '_') and hdr not in (
                        'to', 'cc', 'bcc', 'from', 'subject',
                        'mime-version', 'content-type', 'content-transfer-encoding',
                        'received',
                        'tags', 'x-email'):
                    misc_headers.append(
                        format_header(hdr, draft[hdr]).replace(': ', ':'))
            self.options['--header='][:0] = misc_headers

            want_message = not self.options.get('--message=')
            for part in draft.get('_PARTS', []):
                ctype = part.get('content-type', ['text/plain'])[0]
                cdisp = part.get('content-disposition', ['inline', {}])
                if ('_TEXT' in part) and (cdisp[0] == 'inline'):
                    if not want_message:
                        pass
                    elif ctype == 'text/html':
                        self.options['--html='].append(part['_TEXT'])
                    else:
                        self.options['--text='].append(part['_TEXT'])
                elif '_DATA' in part:
                    self.attachments.append((
                        ctype,
                        os.path.basename(cdisp[1].get('filename', 'data.bin')),
                        base64.b64decode(part['_DATA'])))
    ...
    async def do_render_result(self):
      try:
        ...
        if fmt in ('text', 'rfc822', 'rfc2822'):
            ...
            # FIXME: Include the rfc2822 in the JSON?
            self.print_json(as_tree)
        elif fmt == 'sexp':
            # FIXME: Include the rfc2822 in the SEXP?
            self.print_sexp(as_tree)
        else:
            raise Nonsense('Unsupported output format: %s' % fmt)
      except:
    ...
    async def run(self):
        ...
        if 'bounce' in self.options.get('--forwarding='):
            if (len(self.forwarding) > 1
                    or len(self.options.get('--forward=')) > 1):
                raise Nonsense('Please only bounce/resend one email at a time.')
