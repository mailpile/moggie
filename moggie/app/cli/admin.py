...
class CommandAutotag(CLICommand):
    ...
    matching the search terms. The default spam/trash suppression will
    ...
    async def run(self):
        ...
        worker = self.worker.connect()
        search_res = await self.perform_search(worker, ctx)
        atag_res = await worker.async_api_request(self.access,
...
class CommandAutotagTrain(CommandAutotag):
    """moggie autotagtrain [<tag1> [<tag2> ...]] -- <search terms>

    Train the autotagger(s) for the named tags, using the messages
    matching the search terms. The default spam/trash suppression will
    be disabled for the search.
    ...
    Whether the matching messages are considered to be "spam" or "ham"
    for the purposes of training depends on whether they have the
    relevant tags or not. E.g. if training the "spam" tag, any messages
    already tagged as "spam" will be considered as positive samples
    and any messages without will be treated as negative.
    ...
    async def run(self):
        ...
        results = []
        for ctx in contexts:
            sres = await self.perform_search(worker, ctx)
            treq = RequestAutotagTrain(context=ctx, tags=self.tags, search=sres)
            results.append(await worker.async_api_request(self.access, treq))
